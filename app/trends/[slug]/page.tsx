...
export default async function TrendDetails({ params }: Props) {
  ...
  const relatedProducts = (() => {
    ...
    const categoryMatches = products.filter((product) => {
      ...
      const matchCategory =
        categoryName?.toLowerCase() === trend.focusArea?.toLowerCase() ||
        product.category.slug === trend.focusAreaSlug;

      const matchTags = trend.hashtags?.some(
        (tag: string) =>
